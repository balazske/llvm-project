...
public:
  ...
  /// statement.
  bool AllowNonConditionErrnoRead = true;
...
static bool isInCondition(const Stmt *S, CheckerContext &C) {
  ...
  while (S && !CondFound) {
    ...
    case Expr::BinaryConditionalOperatorClass:
      CondFound = (S == cast<BinaryConditionalOperator>(ParentS)->getCond());
      break;
    default:
      break;
    }
...
void ErrnoChecker::checkLocation(SVal Loc, bool IsLoad, const Stmt *S,
  ...
  if (IsLoad) {
    ...
    case Errno_MustNotBeChecked:
      ...
      if (!AllowNonConditionErrnoRead || isInCondition(S, C)) {
...
void ento::registerErrnoChecker(CheckerManager &mgr) {
  ...
  Checker->AllowNonConditionErrnoRead =
      Opts.getCheckerBooleanOption(Checker, "AllowNonConditionErrnoRead");
}

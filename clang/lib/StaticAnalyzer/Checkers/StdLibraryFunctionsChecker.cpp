...
class StdLibraryFunctionsChecker
  ...
  public:
    SuccessErrnoConstraint()
        : ErrnoConstraintKind(errno_modeling::Errno_MustNotBeChecked) {}
  ...
  public:
    NoErrnoConstraint()
        : ErrnoConstraintKind(errno_modeling::Errno_Irrelevant) {}
  ...
  public:
    ...
    Summary &Case(const ConstraintSet &CS, const ErrnoConstraintKind &ErrnoC,
                  StringRef Note = "") {
...
private:
  ...
  void applyConstraints(ProgramStateRef State, const CallEvent &Call,
                        CheckerContext &C, const Summary &Summary) const;
...
void StdLibraryFunctionsChecker::applyConstraints(
    ProgramStateRef State, const CallEvent &Call, CheckerContext &C,
    const Summary &Summary) const {
  ...
  for (const SummaryCase &Case : Summary.getCases()) {
    ...
    if (NewState && Summary.getInvalidationKd() == EvalCallAsPure)
...
void StdLibraryFunctionsChecker::checkPostCall(const CallEvent &Call,
  ...
  applyConstraints(C.getState(), Call, C, *FoundSummary);
}
...
bool StdLibraryFunctionsChecker::evalCall(const CallEvent &Call,
  ...
  case EvalCallAsPure: {
    ...
    applyConstraints(State, Call, C, Summary);
...
void StdLibraryFunctionsChecker::initFunctionSummaries(
  ...
  if (ModelPOSIX) {
    ...
    addToFunctionSummaryMap(
        ...
        Summary(EvalCallAsPure)
            .Case(ReturnsNonnegative, ErrnoMustNotBeChecked)
            .Case(ReturnsMinusOne, ErrnoNEZeroIrrelevant)
            .ArgConstraint(
                ArgumentCondition(0, WithinRange, Range(0, IntMax))));
